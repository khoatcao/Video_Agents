...
def get_oauth_url(state: str = "") -> str:
    """
    Build the Facebook OAuth authorisation URL with the required page scopes.

    Open the returned URL in a browser. After the user authorises, Facebook
    redirects to FACEBOOK_REDIRECT_URI with a `code` query parameter — pass
    that code to exchange_code_for_token().
    """
    params = {
...
def exchange_code_for_token(code: str) -> str:
    """
    Exchange the OAuth authorisation code for a short-lived user access token.

    Args:
        code: The `code` value from the OAuth redirect query string.

    Returns:
        Short-lived user access token (valid ~1 hour).
    """
    resp = requests.get(
...
def get_long_lived_token(short_lived_token: str) -> str:
    ...
    Args:
        short_lived_token: Token returned by exchange_code_for_token().
...
def get_page_access_token(long_lived_user_token: str, page_id: str | None = None) -> str:
    ...
    Args:
        long_lived_user_token: Token from get_long_lived_token().
        page_id: Page ID to fetch the token for; defaults to FACEBOOK_PAGE_ID.

    Returns:
        Page access token (does not expire while the app is connected).
    """
